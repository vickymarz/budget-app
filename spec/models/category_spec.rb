...
RSpec.describe Category, type: :model do
  ...
  it 'valid if name is present' do
    @user = User.create!(name: 'okorojiebube', email: 'okorojiebube2@gmail.com', password: 1234567)
    ...
    @user = User.create!(name: 'okorojiebube', email: 'okorojiebube2@gmail.com', password: 1234567)
    @category = Category.create( icon: 'www.pic.com', author_id: @user.id)
    expect(@category).to_not be_valid
  end

  it 'Must be associated with a user' do
    @category = Category.create( name: 'Bitcoin', icon: 'www.pic.com')
    expect(@category).to_not be_valid
  end
  ...
  it 'invalid if icon is absent' do
    @user = User.create!(name: 'okorojiebube', email: 'okorojiebube2@gmail.2com', password: 1234567)
